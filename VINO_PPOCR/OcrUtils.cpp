...
void getInputName(const ov::Model& network,const char *&inputName) {
    size_t numInputNodes = network.inputs().size();
        //session->GetInputCount();
    if (numInputNodes > 0) {
       std::string t = network.inputs()[0].get_any_name();
       inputName = t.data();
        //Ort::AllocatorWithDefaultOptions allocator;
        //{
...
void getOutputName(const ov::Model& network,const char *&outputName) {
    size_t numOutputNodes = network.outputs().size();//session->GetInputCount();
    if (numOutputNodes > 0) {
        std::string t = network.outputs()[0].get_any_name();
        outputName = t.data();
        //Ort::AllocatorWithDefaultOptions allocator;
        //{
